...
class User extends Authenticatable
{
    ...
    public function hasPrivilege(string $slug): bool
    {
        ...
        // The trait's hasPrivilege() checks tyroPrivilegeSlugs() which is cached.
        // Admin/super-admin bypass is handled in hasPrivilege() via the '*' wildcard role.
        // $userPrivileges = $this->tyroPrivilegeSlugs();
        // return in_array($slug, $userPrivileges, true) || in_array('*', $userPrivileges, true);
    }


    // hasRole() is provided by HasTyroRoles trait (supports cache + wildcard '*' role)
    // Do NOT override it here — removing the override restores correct isSuperAdmin() behavior
