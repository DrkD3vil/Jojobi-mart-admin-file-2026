...
class CustomerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            // due_balance / advance_balance / reward_points are intentionally
            // omitted here: they're not mass-assignable (see Customer model)
            // and already default to 0 at the schema level for a new row.
    ...
    public function destroy(Customer $customer)
    {
        try {
            ...
            // customer_balances/customer_reward_ledgers cascade-delete with the
            // customer (see migrations), which would silently wipe any owed
            // balance or ledger history. Block deletion instead so a nonzero
            // due/advance balance or reward point total (or any ledger entry)
            // can't just disappear.
            $hasOutstandingBalance = (float) $customer->due_balance > 0
                || (float) $customer->advance_balance > 0
                || (float) $customer->reward_points > 0;

            if ($hasOutstandingBalance || $customer->balanceLedgers()->exists() || $customer->rewardLedgers()->exists()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Cannot delete customer with a non-zero balance, reward points, or existing ledger history.'
                ], 422);
            }
    ...
    public function postBalance(Request $request, Customer $customer)
    {
        ...
        return DB::transaction(function () use ($customer, $data) {
            ...
            // Update balance. Lock the customer row so two concurrent balance
            // postings (e.g. two terminals hitting the same customer at once)
            // can't both read the same stale balance and clobber each other's
    ...
    public function postRewards(Request $request, Customer $customer)
    {
        ...
        return DB::transaction(function () use ($customer, $data) {
            ...
            // Lock the customer row for the duration of this check-then-write
            // so two concurrent reward postings can't both pass the "enough
            // points" check against the same stale balance and both apply
            // (which would let redemptions push the balance negative) or
            // otherwise lose one of the updates.
            $customer = Customer::where('id', $customer->id)->lockForUpdate()->firstOrFail();
