    ...
    <div class="expense-container" style="min-height: calc(100vh - var(--header-height));">
        ...
        <div class="category-breakdown">
            ...
            <div class="breakdown-grid">
                @php
                    // $categoryTotals is computed server-side from the full filtered
                    // query (not just the current paginated page of $expenses), so
                    // these percentages actually add up against $filteredTotal.
                    $colors = ['#4F46E5', '#7C3AED', '#EC4899', '#EF4444', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#F472B6', '#34D399'];
                    $maxCategoryTotal = $categoryTotals->max() ?: 1;
                @endphp
                @foreach($categoryTotals as $catName => $total)
                    <div class="breakdown-item">
                        ...
                        <div class="breakdown-bar-container">
                            @php
                                $percentage = ($total / $maxCategoryTotal) * 100;
                            @endphp
                            <div class="breakdown-bar" style="width: {{ $percentage }}%; background-color: {{ $colors[$loop->index % count($colors)] }};">
                                <span class="breakdown-percentage">{{ $filteredTotal > 0 ? round(($total / $filteredTotal) * 100, 1) : 0 }}%</span>
                            </div>
                        </div>
                    </div>
                @endforeach
