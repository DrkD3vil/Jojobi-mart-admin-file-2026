{{-- resources/views/products/product-batches/create.blade.php --}}
@extends('layouts.app')
...
@section('content')
    ...
    <div class="pb-page" id="pbCreateRoot" data-products-search="{{ $urlProductsSearch }}"
        data-gift-search="{{ $urlGiftSearch }}" data-batches-by-product="{{ $urlBatchesJsonByProduct }}"
        data-batch-json="{{ $urlBatchJson }}">
        {{-- Toasts --}}
        @if (session('error'))
        ...
        <div class="pb-header">
            <div>
                ...
                <h2 class="pb-title">➕ Add New Product Batch</h2>
                <p class="pb-subtitle">Fast entry with copy-batch, smart hints, and live search.</p>
            </div>

            <div class="pb-header__actions">
                @if (isset($selectedProduct) && $selectedProduct)
                    <a class="pb-btn pb-btn--ghost" href="{{ route('products.edit', $selectedProduct) }}">
                        <svg viewBox="0 0 24 24">
                ...
                @else
                    <a class="pb-btn pb-btn--ghost" href="{{ route('products.index') }}">
                        <svg viewBox="0 0 24 24">
                            <path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z" />
                        </svg>
                        Back to Products
                ...
                <button type="button" class="pb-btn pb-btn--secondary" id="pbOpenCopyDrawer" disabled>
                    <svg viewBox="0 0 24 24">
                        <path
            ...
            @if (isset($selectedProduct) && $selectedProduct)
                <div class="pb-selected">
                    <div class="pb-selected__left">
                        ...
                        <div>
                            <div class="pb-selected__title">Selected Product</div>
                            <div class="pb-selected__meta">
                                <span class="pb-chip"><strong>{{ $selectedProduct->name }}</strong></span>
                                <span class="pb-chip">Barcode: {{ $selectedProduct->barcode }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="pb-selected__right">
                        <div class="pb-hint">
                            Tip: Use “Copy Another Batch” to fill pricing/wholesale instantly.
                        </div>
                    </div>
                </div>
            @endif

            <form method="POST" action="{{ route('product.batches.store') }}" class="pb-form" id="pbBatchForm" novalidate>
                @csrf

                {{-- Hidden Product ID --}}
                @if (isset($selectedProduct) && $selectedProduct)
                    <input type="hidden" name="product_id" id="product_id" value="{{ $selectedProduct->id }}">
                @else
                ...
                @if (!isset($selectedProduct) || !$selectedProduct)
                    <div class="pb-section">
                        <div class="pb-section__head">
                            ...
                            <p class="pb-section__desc">Search by name or barcode. Then you can copy an existing batch.</p>
                        </div>

                        <div class="pb-grid pb-grid--2">
                            <div class="pb-field" data-tip="Type at least 2 characters. Use barcode for fastest match.">
                                ...
                                @enderror
                                <div class="pb-help">After selecting a product, the “Copy Another Batch” button becomes
                                    available.</div>
                            </div>

                            <div class="pb-field" data-tip="Fallback dropdown. Good when you already know the product.">
                ...
                <div class="pb-section">
                    <div class="pb-section__head">
                        ...
                        <p class="pb-section__desc">Stock + unit info. Keep it consistent for reporting.</p>
                    </div>
                    ...
                    <div class="pb-grid pb-grid--3">
                        <div class="pb-field" data-tip="Optional. Helpful for vendor tracking.">
                            ...
                            <div class="pb-control">
                                <input type="text" class="pb-input" name="batch_no" value="{{ old('batch_no') }}"
                                    placeholder="e.g. BATCH-001">
                            </div>
                            @error('batch_no')
                                <div class="pb-error">{{ $message }}</div>
                            @enderror
                            <div class="pb-help">You can leave this empty if you don’t track vendor batch no.</div>
                        </div>

                        <div class="pb-field" data-tip="Supports decimals like 0.100.">
                            <label class="pb-label required">Quantity</label>
                            <div class="pb-control">
                                <input type="number" class="pb-input" name="quantity" value="{{ old('quantity') }}"
                                    required min="0.001" step="any">
                            </div>
                            @error('quantity')
                                <div class="pb-error">{{ $message }}</div>
                            @enderror
                            <div class="pb-help">Use decimal quantity for kg/liter style inventory.</div>
                        </div>

                        <div class="pb-field" data-tip="Select unit of measurement">
    <label class="pb-label required">Unit</label>

    <div class="pb-control">
        <select name="unit" class="pb-input" required>
            @php
                $units = [
                    'pcs'   => 'Pieces (pcs)',
                    'dozen' => 'Dozen',
    ...
    <div class="pb-help">
        Keep units consistent to avoid stock confusion.
    </div>
</div>
                        ...
                        <div style="margin-top:12px;">
                            <div class="pb-grid pb-grid--2">
                                <div class="pb-field"
                                    data-tip="Choose where the initial stock will be stored (warehouse/store).">
                                    <label class="pb-label required">Stock Location</label>
                                    <div class="pb-control">
                                        <select name="location_id" class="pb-select" required>
                                            <option value="">-- Select Location --</option>
                                            @foreach ($locations ?? [] as $loc)
                                                <option value="{{ $loc->id }}" @selected(old('location_id', $defaultLocationId ?? null) == $loc->id)>
                                    ...
                                    @error('location_id')
                                        <div class="pb-error">{{ $message }}</div>
                                    @enderror
                                    <div class="pb-help">Your batch quantity will be added to this location in
                                        batch_stocks.</div>
                                </div>

                                <div class="pb-field"
                                    data-tip="If you keep stock in multiple stores, you can later transfer stock between locations.">
                                    <label class="pb-label">Tip</label>
                                    <div class="pb-help" style="margin-top:.25rem;">
                                        - Create batches into <strong>Warehouse</strong><br>
                                        - Transfer to <strong>Store</strong> when needed<br>
                                        - POS will deduct from Store location
                                    </div>
                                </div>
                    ...
                    {{-- PRICING (keep your discount logic unchanged) --}}
                    <div class="pb-section">
                        <div class="pb-section__head">
                            <h4 class="pb-section__title">💰 Pricing Configuration</h4>
                            <p class="pb-section__desc">Each price input has boxed border + hint. Discount logic stays
                                unchanged.</p>
                        </div>
                            ...
                            <div class="pb-field pb-pricebox" data-tip="Your purchase cost per unit.">
                                ...
                                <div class="pb-control pb-control--money">
                                    <input type="number" class="pb-input" name="buy_price"
                                        value="{{ old('buy_price') }}" required min="0" step="0.01"
                                        placeholder="0.00">
                                ...
                                @error('buy_price')
                                    <div class="pb-error">{{ $message }}</div>
                                @enderror
                                <div class="pb-help">Used for stock valuation and profit calculations.</div>
                            </div>
                            ...
                            <div class="pb-field pb-pricebox" data-tip="Original price before any discount.">
                                ...
                                <div class="pb-control pb-control--money">
                                    <input type="number" class="pb-input" name="original_sell_price"
                                        value="{{ old('original_sell_price') }}" required min="0" step="0.01"
                                        placeholder="0.00">
                                    <span class="pb-suffix">Tk.</span>
                                </div>
                                @error('original_sell_price')
                                    <div class="pb-error">{{ $message }}</div>
                                @enderror
                                <div class="pb-help">Base price used to compute sell price.</div>
                            </div>

                            <div class="pb-field pb-pricebox"
                                data-tip="If you set discounted price, discount % will lock.">
                                <label class="pb-label">Discounted Price</label>
                                <div class="pb-control pb-control--money">
                                    <input type="number" class="pb-input" name="discounted_price" id="discounted_price"
                                        value="{{ old('discounted_price', 0) }}" min="0" step="0.01"
                                        placeholder="0.00">
                                    <span class="pb-suffix">Tk.</span>
                                </div>
                                @error('discounted_price')
                                    <div class="pb-error">{{ $message }}</div>
                                @enderror
                                <div class="pb-help">Set this OR set discount % (not both).</div>
                            </div>

                            <div class="pb-field pb-pricebox"
                                data-tip="If you set discount %, discounted price will lock.">
                                ...
                                @error('discount_percentage')
                                    <div class="pb-error">{{ $message }}</div>
                                @enderror
                                <div class="pb-help">Use percentage discount instead of fixed discounted price.</div>
                            </div>
                    ...
                    <div class="pb-section">
                        <div class="pb-section__head">
                            ...
                            <p class="pb-section__desc">For dealers/shops pricing tiers.</p>
                        </div>
                            ...
                            <div class="pb-field" data-tip="Dealer wholesale selling price.">
                                ...
                                <div class="pb-control pb-control--money">
                                    <input type="number" class="pb-input" name="whole_sell_price"
                                        value="{{ old('whole_sell_price') }}" min="0" step="0.01"
                                        placeholder="0.00">
                                    <span class="pb-suffix">Tk.</span>
                                </div>
                            </div>
                            <div class="pb-field" data-tip="Minimum quantity to allow wholesale price.">
                                ...
                                <div class="pb-control">
                                    <input type="number" class="pb-input" name="whole_sell_min_qty"
                                        value="{{ old('whole_sell_min_qty') }}" min="0" step="any"
                                        placeholder="0">
                                </div>
                            </div>
                            <div class="pb-field" data-tip="Maximum qty for wholesale tier (optional).">
                                ...
                                <div class="pb-control">
                                    <input type="number" class="pb-input" name="whole_sell_max_qty"
                                        value="{{ old('whole_sell_max_qty') }}" min="0" step="any"
                                        placeholder="0">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="pb-field" data-tip="Customer bulk selling price.">
                                ...
                                <div class="pb-control pb-control--money">
                                    <input type="number" class="pb-input" name="customer_whole_price"
                                        value="{{ old('customer_whole_price') }}" min="0" step="0.01"
                                        placeholder="0.00">
                                    <span class="pb-suffix">Tk.</span>
                                </div>
                            </div>
                            <div class="pb-field" data-tip="Minimum qty to allow bulk price.">
                                ...
                                <div class="pb-control">
                                    <input type="number" class="pb-input" name="customer_whole_min_qty"
                                        value="{{ old('customer_whole_min_qty') }}" min="0" step="any"
                                        placeholder="0">
                                </div>
                            </div>
                            <div class="pb-field" data-tip="Maximum qty for bulk tier (optional).">
                                ...
                                <div class="pb-control">
                                    <input type="number" class="pb-input" name="customer_whole_max_qty"
                                        value="{{ old('customer_whole_max_qty') }}" min="0" step="any"
                                        placeholder="0">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="pb-section">
                        <div class="pb-section__head">
                            ...
                            <p class="pb-section__desc">Enable gift offer per batch. Fast gift product search included.
                            </p>
                        </div>

                        <div class="pb-grid pb-grid--2">
                            <div class="pb-field" data-tip="Turn ON only when you want a buy X get Y gift.">
                                ...
                                <label class="pb-toggle">
                                    ...
                                    <input type="checkbox" id="is_free_offer_active" name="is_free_offer_active"
                                        value="1" {{ old('is_free_offer_active', 0) ? 'checked' : '' }}>
                                ...
                                <div class="pb-control">
                                    <select class="pb-select" id="pbGiftSelectFallback">
                                        ...
                                        @foreach ($giftProducts as $gp)
                                            <option value="{{ $gp->id }}" @selected(old('free_product_id') == $gp->id)>
                            ...
                            <div class="pb-grid pb-grid--3">
                                <div class="pb-field" data-tip="Buy quantity required to trigger gift.">
                                    ...
                                    <div class="pb-control">
                                        <input type="number" class="pb-input" name="free_buy_qty"
                                            value="{{ old('free_buy_qty', 1) }}" min="0.0001" step="any"
                                            placeholder="1">
                                    </div>
                                    @error('free_buy_qty')
                                        <div class="pb-error">{{ $message }}</div>
                                    @enderror
                                    <div class="pb-help">Example: Buy 2 → qualifies.</div>
                                </div>
                                ...
                                <div class="pb-field" data-tip="How many gift units to give per rule.">
                                    ...
                                    <div class="pb-control">
                                        <input type="number" class="pb-input" name="free_qty"
                                            value="{{ old('free_qty', 1) }}" min="0.0001" step="any"
                                            placeholder="1">
                                    </div>
                                    @error('free_qty')
                                        <div class="pb-error">{{ $message }}</div>
                                    @enderror
                                    <div class="pb-help">Example: Get 1 free.</div>
                                </div>
                                ...
                                <div class="pb-field pb-giftpreview">
                                    ...
                                    <div class="pb-previewbox">
                                        ...
                                        <div class="pb-previewbox__row">
                                            <span>Buy</span> <strong id="pbPrevBuy">1</strong>
                                        </div>
                                        <div class="pb-previewbox__row">
                                            <span>Get</span> <strong id="pbPrevFree">1</strong>
                                        </div>
                                        <div class="pb-previewbox__row">
                                            <span>Gift</span> <strong id="pbPrevGiftName">Not selected</strong>
                                        </div>
                                    </div>
                                    <div class="pb-help">This is only preview. Actual stock validation happens in
                                        backend.</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="pb-section">
                        ...
                        <div class="pb-grid pb-grid--2">
                            ...
                            <div class="pb-field" data-tip="Optional. Helps prevent selling expired stock.">
                                ...
                                <div class="pb-control">
                                    <input type="date" class="pb-input" name="expiry_date"
                    ...
                    <div class="pb-footer">
                        ...
                        <div class="pb-footer__right">
                            <a href="{{ isset($selectedProduct) ? route('products.edit', $selectedProduct) : route('products.index') }}"
            ...
            <div class="pb-drawer__panel">
                ...
                <div class="pb-drawer__head">
                    <div>
                        ...
                        <div class="pb-drawer__desc">
                            Search any product → select a batch → copy into this form.
                        </div>
                    </div>
                ...
                <div class="pb-drawer__foot">
                    <div class="pb-hint">
                        Tip: click batch or use ↑ ↓ Enter. Expired batches show warning.
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const root = document.getElementById('pbCreateRoot');
            const cfg = {
            ...
            function highlightSuggestActive(el, idx) {
                ...
                items.forEach((it, i) => {
                    it.style.background = (i === idx) ? 'var(--accent)' : '';
                });
            ...
            function enableCopyBtnIfSelected() {
                const ok = productIdInput && String(productIdInput.value || '').trim() !== '';
                if (openDrawerBtn) openDrawerBtn.disabled = !ok;
            }

            function renderSelectedProduct(p) {
                if (!selectedLine) return;
                setHTML(selectedLine, `
        ...
        `);
            ...
            enableCopyBtnIfSelected();
            if (productIdInput?.value) openDrawerBtn.disabled = false;

            // -------------------------
            // Gift offer + gift search
            // -------------------------
        ...
        `;
            ...
            // -------------------------
            // Drawer: product search + batch list + keyboard nav + copy modes
            // -------------------------
            ...
            let drawerBatchItems = []; // DOM items
            ...
            function openDrawer() {
                ...
                // default: if main product selected -> load
                const pid = String(productIdInput?.value || '').trim();
                if (pid) {
                    loadBatchesForProduct(pid, `Product #${pid}`);
                } else {
                    drawerList.innerHTML = `<div class="pb-skel">Search a product to load batches…</div>`;
                }

                // focus search
                setTimeout(() => drawerProductSearch?.focus(), 50);
            }
            ...
            document.addEventListener('keydown', (e) => {
                ...
                if (e.key === 'ArrowDown' || e.key === 'ArrowUp' || e.key === 'Enter') {
                    if (!drawerBatchItems.length) return;

                    // only if NOT typing inside input
                    const active = document.activeElement;
                    const isTyping = active && (active.tagName === 'INPUT' || active.tagName ===
                        'TEXTAREA' || active.tagName === 'SELECT');
                    if (isTyping && active.id === 'pbDrawerProductSearch') return;

                    if (e.key === 'ArrowDown') {
                        e.preventDefault();
                        drawerActiveBatchIndex = Math.min(drawerBatchItems.length - 1,
                            drawerActiveBatchIndex + 1);
                        highlightDrawerBatchActive();
                    } else if (e.key === 'ArrowUp') {
                        e.preventDefault();
                        drawerActiveBatchIndex = Math.max(0, drawerActiveBatchIndex - 1);
                        highlightDrawerBatchActive();
                    } else if (e.key === 'Enter') {
                        e.preventDefault();
                        drawerBatchItems[drawerActiveBatchIndex]?.click();
                    }
                }
            });
            ...
            $('#pbApplyDrawerProduct')?.addEventListener('click', () => {
                ...
                if (typeof renderSelectedProduct === 'function') renderSelectedProduct(
            ...
            async function loadBatchesForProduct(pid, productName) {
                ...
                if (!pid || !cfg.batchesByProduct) return;
                ...
                <div class="pb-batchitem__top">
                    <div class="pb-batchitem__sku">
                        ${escapeHtml(b.batch_sku || ('Batch #' + b.id))} ${badge}
                    </div>
                    <div class="pb-batchitem__qty">
                        ${escapeHtml(String(b.quantity ?? ''))} ${escapeHtml(b.unit || '')}
                    </div>
                </div>
            ...
            `;
                    ...
                    div.addEventListener('click', async () => {
                        // auto apply drawer product to form before copy (optional)
                        if (drawerSelectedProduct?.id) {
                            productIdInput.value = drawerSelectedProduct.id;
                            if (productManual) productManual.value = drawerSelectedProduct
                                .id;
                            if (typeof renderSelectedProduct === 'function')
            ...
            // -------------------------
            // Apply batch to form (Advanced Copy Modes)
            // -------------------------
            async function applyBatchToFormAdvanced(batch, mode = 'all') {
                ...
                let keys = [];
                if (mode === 'all') {
                    keys = ['pricing', 'wholesale', 'gift', 'dates'];
                } else {
                    keys = [mode];
                }
                ...
                keys.forEach(group => {
                    ...
                    Object.keys(map).forEach(k => {
                        ...
                        // special: free_product_id is hidden
                        ...
                        // smart: if unit empty, use batch unit
                        if (name === 'unit' && el.value && String(el.value).trim() !== '')
                            return;
                ...
                // booleans (only in ALL mode)
                if (mode === 'all') {
                    ['is_online', 'is_offline', 'is_pos', 'is_active'].forEach(k => {
                        const el = document.querySelector(`[name="${k}"]`);
                        if (el && el.type === 'checkbox') el.checked = !!batch[k];
                    });
                }

                // gift toggle if mode includes gift
                if (mode === 'all' || mode === 'gift') {
                    if (offerToggle) offerToggle.checked = !!batch.is_free_offer_active;
                    syncOfferBox();
                }

                // ✅ re-run pricing calc (your discount script listens on inputs)
            ...
            // Draft save/restore
            // -------------------------
            const DRAFT_KEY = 'pb_create_draft_v2';
            ...
            try {
                ...
                if (saved && !{{ $errors->any() ? 'true' : 'false' }}) {
                    const obj = JSON.parse(saved);
                    const selectedLocked = {{ isset($selectedProduct) && $selectedProduct ? 'true' : 'false' }};
                    if (selectedLocked) delete obj.product_id;
                    setFormData(obj);
                }
            } catch (e) {}
