...
class OrderController extends Controller
{
        ...
        return response()->json([
            'rows' => $orders->map(function ($order) {
                return [
                    ...
                    'can_edit' => in_array($order->status, ['pending', 'processing', 'cancel']),
    ...
    public function addItem(Request $request, Order $order)
    {
        ...
        return DB::transaction(function () use ($request, $order, $data, $locationId) {
            ...
            $batch = ProductBatch::with(['product:id,name,barcode'])->findOrFail($data['batch_id']);
            $saleUnit = $data['unit'] ?? ($batch->unit ?? 'pcs');
            $qtyBatch = $data['quantity'];
            ...
            if (!$stock || (float)$stock->on_hand < $qtyBatch) {
                return response()->json([
                    'success' => false,
            ...
            $payload = [
                ...
                'unit' => $saleUnit,
                'qty_in_batch_unit' => (float) $qtyBatch,
    ...
    private function getOrderStatistics()
    {
        return [
            ...
            'total_revenue' => Order::withoutTrashed()->where('status', '!=', 'cancelled')->sum('payable_total'),
    ...
    {
        $subtotal = (float) OrderItem::where('order_id', $order->id)->sum('total_price');
