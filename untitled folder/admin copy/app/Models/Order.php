...
class Order extends Model
{
    ...
    */
    ...
    {
        return $this->split_status === 'split_parent';
    }

    public function isSplitChild()
    {
        return $this->is_split_child && $this->split_status === 'split_child';
    }
    ...
    */
    ...
    public function canSplit(): bool
    {
        return $this->isOriginal()
            && in_array($this->status, ['pending', 'processing', 'paid', 'partial', 'completed'])
            && $this->items()->count() > 0
            && !$this->is_split_child;
    }
    ...
    */
    ...
    public function calculateProfit(): array
    {
        ...
        foreach ($this->items as $item) {
            $batch = $item->batch;
            if ($batch) {
                $costPerUnit = $batch->cost_price ?? 0;
            } else {
                $costPerUnit = $item->product?->cost_price ?? 0;
            }
