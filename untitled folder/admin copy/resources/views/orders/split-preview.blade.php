...
<div class="split-preview">
    <div class="alertx alertx-warning">
            ...
            <tbody>
                @foreach($splitItems as $item)
                    <tr>
                        ...
                        <td>
                            <span class="fw-bold">{{ number_format($item['quantity'], 3) }}</span>
                            <div class="text-muted small">Available: {{ number_format($item['available_quantity'], 3) }}</div>
                        </td>
                        <td>{{ currency_bdt($item['unit_price']) }}</td>
                        <td class="text-end fw-bold">{{ currency_bdt($item['total_price']) }}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr style="border-top: 2px solid var(--border);">
                    <th colspan="4" class="text-end">Total to Split:</th>
                    <th class="text-end" style="font-size:18px; color: var(--warning);">
                        {{ currency_bdt($totalAmount) }}
                    </th>
                </tr>
            </tfoot>
