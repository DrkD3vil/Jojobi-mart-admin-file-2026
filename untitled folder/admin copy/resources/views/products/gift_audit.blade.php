...
@section('content')
    <div class="container mx-auto px-4 py-6 max-w-7xl">
        ...
        <!-- Filters Section -->
<div class="bg-card rounded-xl p-4 mb-6 shadow-card border border-border" style="
...
">
    <form method="GET" action="{{ route('product.gift-audit') }}" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <!-- Search Input -->
            <div class="relative">
                <i class="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2" style="
                    color: var(--muted-foreground);
                "></i>
                <input type="text" name="search" placeholder="Search offers..." value="{{ $searchTerm }}" style="
                    width: 100%;
                    padding-left: 2.5rem;
                    padding-right: 1rem;
                    padding-top: 0.5rem;
                ...
                " onfocus="
                    ...
                    this.style.boxShadow = '0 0 0 2px var(--ring)';
            ...
            ">
                ...
                ">
                    ...
                    <label style="
                        ...
                        cursor: pointer;
                    ">
                        <input type="checkbox" name="active_only" value="1" style="
                            border-radius: 0.25rem;
                            border: 1px solid var(--border);
                            color: var(--primary);
                ...
                ">
                    ...
                    <a href="{{ route('product.gift-audit') }}" style="
                        padding-left: 1rem;
                        padding-right: 1rem;
                        padding-top: 0.5rem;
                        padding-bottom: 0.5rem;
        ...
        <div class="bg-card rounded-xl shadow-card border border-border overflow-hidden">
            @if (count($productGiftDetails) > 0)
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-border bg-secondary/30">
                                <th
                                    class="px-6 py-4 text-left text-sm font-semibold text-foreground uppercase tracking-wider">
                                    Product</th>
                                <th
                                    class="px-6 py-4 text-left text-sm font-semibold text-foreground uppercase tracking-wider">
                                    Batch SKU</th>
                                ...
                                <tr class="hover:bg-accent/30 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                class="flex-shrink-0 h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center">
                                                <i class="fas fa-box text-primary"></i>
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-foreground">
                                                    {{ $detail['product_name'] }}</div>
                                                <div class="text-xs text-muted-foreground">{{ $detail['product_sku'] }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="px-3 py-1 rounded-full text-xs font-medium bg-blue-500/10 text-blue-500">
                                            {{ $detail['batch_sku'] }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                class="flex-shrink-0 h-8 w-8 rounded-lg bg-green-500/10 flex items-center justify-center mr-3">
                                                <i class="fas fa-gift text-green-500 text-sm"></i>
                                            </div>
                                            <div>
                                                <span
                                                    class="text-sm text-foreground">{{ $detail['free_product_name'] }}</span>
                                                <div class="text-xs text-muted-foreground">
                                                    {{ $detail['free_product_sku'] }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-lg font-bold text-foreground">{{ $detail['free_qty_per_offer'] }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <span
                                                class="text-sm font-medium text-foreground">{{ $detail['available_free_qty'] }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $statusClass =
                                                $detail['status_class'] == 'high'
                                                    ? 'bg-green-500/10 text-green-500'
                                                    : ($detail['status_class'] == 'medium'
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <a href="{{ route('product.batches.edit', $detail['batch_id']) }}"
                                                class="p-2 rounded-lg border border-border hover:bg-accent text-foreground">
                ...
                <!-- Pagination -->
                <div
                    class="px-6 py-4 border-t border-border bg-secondary/30 flex flex-col md:flex-row md:items-center justify-between">
                    <div class="text-sm text-muted-foreground mb-4 md:mb-0">
                        Showing {{ $productBatches->firstItem() }} to {{ $productBatches->lastItem() }} of
                        {{ $productBatches->total() }} offers
                    </div>
                    <div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                ...
                <div class="bg-card rounded-xl p-6 shadow-card border border-border">
                    ...
                    <div class="space-y-4">
                        @foreach ($locations as $location)
                            @php
                                // Ensure selectedLocation is available before using it
                                $selectedLocation = $selectedLocation ?? null;

                                // Filter product gift details based on the location
                                $locationQty = collect($productGiftDetails)
                                    ->filter(function ($item) use ($location, $selectedLocation) {
                            ...
                            {{-- Show selected location if it matches --}}
                            <div>
                                @if ($selectedLocation == $location->id)
                                    <div class="selected-location font-bold text-primary">
                                ...
                                {{-- Show available quantity if it's greater than 0 --}}
                                @if ($locationQty > 0)
                                    <div class="mt-2">
                                        <div class="flex justify-between text-sm mb-1">
                                        ...
                                        {{-- Progress bar showing available quantity as a percentage of total free quantity --}}
                                        <div class="w-full bg-secondary rounded-full h-2">
    ...
    <div id="editModal" class="fixed inset-0 z-50 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                class="inline-block align-bottom bg-card rounded-xl text-left overflow-hidden shadow-card transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <div class="px-6 py-4">
                    <form id="editForm">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-foreground mb-2">Free Quantity Per
                                    Offer</label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            tableRows.forEach(row => {
                row.addEventListener('click', function(e) {
                    ...
                    if (e.target.closest('button') || e.target.closest('a') || e.target.type ===
                        'checkbox') {
                        return;
                    }
                    ...
                    const productName = this.querySelector('td:first-child .font-medium')
                        .textContent;
                    const batchSku = this.querySelector('td:nth-child(2) span').textContent;
                    const freeProduct = this.querySelector('td:nth-child(3) span.text-sm')
                        .textContent;
